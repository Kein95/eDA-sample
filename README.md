...
| **Clone repo này về làm trang cho khoá học của bạn** | **[Đổi thành khoá của bạn](#đổi-thành-khoá-của-bạn)** ngay dưới đây, đọc trước tiên |

---

## Đổi thành khoá của bạn

Repo này dùng lại được cho khoá học khác. Phần cơ chế (đăng ký, đối soát tiền, phân
quyền, CMS sửa chữ) không dính gì tới nội dung khoá eDA, cứ giữ nguyên.

### ⚠️ Bắt buộc thay trước khi đưa lên mạng

Những thứ dưới đây **không thuộc giấy phép MIT** và không phải của bạn. Xem
[`LICENSE`](LICENSE) mục 2.

| Phải thay | Vì sao |
|---|---|
| `ava/*.jpg` (4 ảnh giảng viên) và tên họ trong tab Giảng viên | **Người thật.** Quyền hình ảnh thuộc về họ. Deploy mà quên thay là trang của bạn đang quảng cáo giảng viên của AI VIETNAM |
| `assets/logo-full.png`, `assets/logo-mark.png` | Nhãn hiệu AI VIETNAM |
| `assets/dongson-*`, `assets/Overview_eDA2026.jpg`, `assets/video/*.jpg` | Hình ảnh của khoá eDA |
| `docs/eDA-2026-Syllabus.md` rồi chạy lại `build-syllabus-html.py` | Giáo trình |
| Chữ trên 5 tab: mô tả khoá, học phí, cam kết, đánh giá học viên | Nội dung khoá eDA |
| Số điện thoại hotline, email, link Google Drive, link đăng ký | Đầu mối liên hệ của người khác |

Cách nhanh nhất để soát chữ: dựng backend xong, vào `/dashboard/web`, tab **Nội dung
trang** liệt kê **toàn bộ** chữ trên cả 5 tab. Sửa ở đó, không phải mở file HTML.

### Chỗ phải sửa trong code

| File | Sửa gì |
|---|---|
| `admin.html` | `SUPABASE_URL`, `SUPABASE_ANON_KEY` (đang là `REPLACE-ME`) |
| `admin.html` dòng 19 | `connect-src` trong CSP, đổi `REPLACE-ME.supabase.co` thành project của bạn |
| `supabase/functions/eda-register/index.ts` | Cùng cặp `REPLACE-ME` |
| `_headers`, `_redirects` | Tên miền, nếu đổi đường dẫn `/admin` `/dashboard` |
| `package.json` | `--project-name eda` trong script `deploy` |
| `TuyenSinh-eDA2026.dc.html` | Đổi tên file cũng được, nhớ sửa theo trong `scripts/dung-thu-muc-xuat-ban.mjs` và `_redirects` |

Chưa điền `REPLACE-ME` thì trang quản trị **tự vào chế độ demo** với dữ liệu mẫu thay vì
treo im. Cứ clone về mở lên xem trước, không cần dựng gì.

### Đổi tên bảng trong database

Migration đặt tiền tố `eda_` cho mọi bảng và hàm. Chạy được luôn, không phải đổi. Muốn
đổi thì tìm thay cả trong `supabase/migrations/`, `supabase/functions/`, `admin.html` và
`scripts/test-*.mjs`, rồi chạy `npm run test:db` để chắc không sót chỗ nào.
...
> thư mục repo**, nghĩa là `/supabase/migrations/*.sql` và `/.git` đều tải về được. Ở nhà thì không sao, ở quán cà phê hay mạng công ty thì ai cũng xem
...
Chạy được ngay, không cần cài gì (không có dependency, `npm install` cũng không cần):

```bash
npm test              # 74 kiểm tra: bảo mật + đối soát + đọc sao kê
npm run build         # dựng bản deploy, báo lỗi nếu thiếu file
```

Gọi thẳng từng bộ cũng được:
...
```bash
npm run test:db       # 60 kiểm tra: dựng Postgres sạch, chạy migration thật,
                      # giả lập đăng nhập từng vai rồi gọi thật
```
...
npm run test:trinh-duyet   # 53 kiểm tra: URL từng tab, F5, Back, ba vai,
                           # sửa chữ trên cả 6 tab, xuất CSV/TXT
```
...
| `assets/` | Logo, hoạ tiết trống đồng, ảnh thumbnail video. **Không thuộc MIT** |
| `ava/` | Ảnh 4 giảng viên thật, `crop-*.jpg` là bản đang dùng. **Không thuộc MIT**, phải thay khi clone |
| `docker/` | nginx + compose để chạy thử |
| `docs/` | Tài liệu dự án, quan trọng nhất là `deployment-guide.md` |
| `scripts/` | Script sinh nội dung, và các bộ kiểm tra |
...
## B3. Sinh lại nội dung bằng script

Hai phần **không gõ tay** mà sinh ra:

```bash
python scripts/build-overview-diagram.py  # sơ đồ tổng quan, ghi thẳng vào trang
python scripts/build-syllabus-html.py     # docs/*.md -> docs/*.html
```

Script tự tìm khối đã chèn lần trước bằng comment mốc và thay đúng khối đó, nên chạy lại
nhiều lần được, không sinh trùng.

Kèm `scripts/audit_white_text.py`: dò chữ trắng nằm trên nền sáng, xem B8.

> Bảng "10 ngành × 10 mảng phân tích" trước đây sinh từ một file Excel giáo trình. File
> đó là tài liệu của khoá eDA nên không đưa vào repo mẫu, và script sinh nó đã bỏ. Bảng
> hiện nằm sẵn trong trang, sửa tay hoặc sửa qua CMS.
...
đọc `.gitignore` (cũng không đọc `.assetsignore`), nên deploy từ gốc là toàn bộ migration
SQL, thư mục `.git` và tài liệu nội bộ đều thành URL công khai. Script trên chép ra đúng
những file trang thật sự cần và **báo lỗi nếu `admin.html` import một file không có trong
danh sách**.
...
- **Mã nguồn** theo MIT: dùng, sửa, bán, deploy thoải mái.
- **Ảnh giảng viên, logo, giáo trình, chữ trên trang** thì **không** cấp phép. Đó là của
  AI VIETNAM và của những người trong ảnh. Clone về phải thay hết, xem
  [Đổi thành khoá của bạn](#đổi-thành-khoá-của-bạn).
- `support.js`, `image-slot.js` sinh bởi toolchain khác, giấy phép theo toolchain đó.

Nhắc lại vì đây là chỗ dễ vấp nhất: MIT nói "dùng không hạn chế" nhưng câu đó chỉ áp cho
phần mềm. Không giấy phép phần mềm nào cho bạn quyền dùng ảnh chân dung của người khác.
